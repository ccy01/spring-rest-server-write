 ...
 * @author ouyang
 * @date 2014-12-04
 * @description virtual deviceId relationtable
 *
 */
public class VrelationHTableMapper implements HTableMapper{
...
//		terminalSettingsBackup(DeviceId, "point2:CompanyId:" + CompanyId + ",DeviceId:" + DeviceId );
	...
	public ResultScanner getResultScannerWithParameterMap(Scan scan) {
		...
		try {
			htable = HbaseTablePool.instance().getHtable(PropUtil.getString("hbase_vrelation_name"));
			try {
	...
	private void terminalSettingsBackup(Short deviceid, String backupstr) {
		...
		try{
			...
			String path = folder + File.separator + "backup" + File.separator + "vrelation" + File.separator 
					+ deviceid + ".txt";
